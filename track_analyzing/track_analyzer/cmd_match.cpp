...
{
void CmdMatch(string const & logFile, string const & trackFile, shared_ptr<NumMwmIds> const & numMwmIds, Storage const & storage, Stat & stat, Stat & matchedStat)
{
...
void CmdMatch(string const & logFile, string const & trackFile)
{
  ...
  Stat stat;
  Stat matchedStat;
  CmdMatch(logFile, trackFile, numMwmIds, storage, stat, matchedStat);
  LOG(LINFO, ("CmdMatch stat.", stat));
}

void UnzipAndMatch(Iter begin, Iter end, string const & trackExt, Stat & stat, Stat & matchedStat)
{
  Storage storage;
  storage.RegisterAllLocalMaps(false /* enableDiffs */);
  shared_ptr<NumMwmIds> numMwmIds = CreateNumMwmIds(storage);
  for (auto it = begin; it != end; ++it)
  {
    ...
    CmdMatch(file, file + trackExt, numMwmIds, storage, stat, matchedStat);
...
void CmdMatchDir(string const & logDir, string const & trackExt)
{
  ...
  vector<Stat> matchedStats(threadsCount);
  auto begin = filesList.begin();
  for (size_t i = 0; i < threadsCount - 1; ++i)
  {
    auto end = begin + blockSize;
    threads[i] = thread(UnzipAndMatch, begin, end, trackExt, ref(stats[i]), ref(matchedStats[i]));
    begin = end;
  }

  UnzipAndMatch(begin, filesList.end(), trackExt, ref(stats[threadsCount - 1]), ref(matchedStats[threadsCount - 1]));
  for (auto & t : threads)
    t.join();
