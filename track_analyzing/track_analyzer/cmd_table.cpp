...
struct RoadInfo
{
  ...
  bool operator<(RoadInfo const & rhs) const
  {
    ...
    if (m_maxspeed != rhs.m_maxspeed)
      return m_maxspeed < rhs.m_maxspeed;
  ...
  uint16_t m_maxspeed = kInvalidSpeed;
...
public:
    ...
    : m_featuresVector(container), m_vehicleModel(vehicleModel)
  {
    ...
    if (container.IsExist(MAXSPEED_FILE_TAG))
      LoadMaxspeeds(container.GetReader(MAXSPEED_FILE_TAG), m_maxspeeds);
  }
...
private:
  RoadInfo GetRoadInfo(Segment const & segment)
  {
    ...
    auto maxspeedValue = kInvalidSpeed;
    if (maxspeed.IsValid())
    {
      maxspeedValue = maxspeed.GetSpeedKmPH(segment.IsForward());
      if (maxspeedValue == kNoneMaxSpeed)
        maxspeedValue = kNoneMaxspeedKMpHValue;

      if (maxspeedValue == kWalkMaxSpeed)
        maxspeedValue = kWalkMaxspeedKMpHValue;

      maxspeedValue = min(maxspeedValue, kNoneMaxspeedKMpHValue);
    }
...
{
  cout << "mwm,hw type,surface type,maxspeed km/h,is city road,is one way,is day,lat lon,distance,time,mean speed km/h\n";
